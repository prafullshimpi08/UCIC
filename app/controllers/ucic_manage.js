...
const getUcicList = async (req, res) => {
  try {
    const result = await ucicService.getUcicList({ ...req.query, ...req.params });
    ...
    console.error("GET UCIC LIST ERROR >>>", err);
    return response.error(req, res, { msgCode: "UCIC_LIST_FETCH_FAILED", data: err }, 500);
  }
};
